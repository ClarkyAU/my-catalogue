...
export const LandingPage = ({ catalogue }) => {
  ...
  Object.entries(catalogue).forEach(([catId, category]) => {
    Object.entries(category.products).forEach(([prodId, product]) => {
      if (product.featured) {
  ...
  return (
    <div className="landing-page">
      ...
      <div className="welcome-message" style={{
        ...
        gap: '15px' /* 👈 This forces a clean, even gap between the paragraphs */
      }}>
        <p style={{ margin: 0, color: '#d1d1d1', fontWeight: 'bold' }}>
          I am currently working on new products, so keep an eye out for updates.
        </p>
        
        <p style={{ margin: 0, color: '#d1d1d1', fontWeight: 'bold' }}>
          Check out the newest products below, or check out the full range above.
        </p>
        
        <p style={{ margin: 0, color: '#d1d1d1', fontWeight: 'bold' }}>
          Want something I don't have listed? Shoot me a message via the order button.
        </p>
      </div>
      {/* -------------------------------------- */}
      ...
      {featuredProducts.length === 0 ? (
        ...
        <div className="product-grid">
          {featuredProducts.map((prod, i) => {
            ...
            return (
              <a key={i} href={`#${prod.categoryId}/${prod.id}`} className="grid-card">
                ...
                <div className="card-details">
                  ...
                  <span className="card-category">{prod.categoryName}</span>
                </div>
              </a>
            );
          })}
